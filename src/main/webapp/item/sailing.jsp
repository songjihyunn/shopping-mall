...
<style>
	...
	<div class="container">
	    ...
	    <form action="search" method="get" style="display: flex; width: 100%; padding-top: 25px;">
	        ...
	        <button type="submit" style="position: absolute; right: 35px; top: -5px; height: 100%; border: none; background-color:transparent;">
	            <img src="/img/search.png" style="width: 20px; height: 20px; padding-top: 20px;">
	        </button>
	    </form>
</div>
<script>
...
<div class="aa">
	...
	<div class="first_box">
    	<c:choose>
			...
			<c:otherwise>
				<div style="display: flex; margin: auto 0; background: #ffffff;">
	...
	<div class="cc">
		<div class="additional-container">
	    	<div style="width: 300px; top: 30px; text-align: left; position: absolute; border: none; overflow: hidden; color: black; font-size: 28px; font-weight: bold;">
			    <c:choose>
			        <c:when test="${product == 10}">
			            상의
			        </c:when>
			        <c:when test="${product == 20}">
			            하의
			        </c:when>
			        <c:when test="${product == 30}">
			            모자
			        </c:when>
			        <c:when test="${product == 40}">
			            양말
			        </c:when>
			        <c:otherwise>
			            악세서리
		    ...
		    <div style="display: flex; gap: 20px; margin-top: 100px;"> <!-- 이미지를 조금 아래로 내렸습니다. -->
		        <% 
		            ArrayList<Item> v = (ArrayList<Item>) request.getAttribute("v"); 
		            int maxItemsToShow = 5; // 최대 표시할 상품 수
		        ...
		        %>
		            <a href="../item/ItemView?it_uid=<%= item.getIt_uid() %>" style="flex: 0 0 auto;">
		                <img src="/upload_item/<%= item.getFile1_thumb() %>" style="width: 250px; height: 250px;">
	...
	<div class="button-container" style="bottom: 20px; position: fixed; right: 20px; border: none; background-color: rgba(255, 255, 255, 0);">
	  <button id="toggleButton" style="border:none; background-color: rgba(255, 255, 255, 0);" >
	  	<img src="/img/bot.png">
	  </button>
	</div>
